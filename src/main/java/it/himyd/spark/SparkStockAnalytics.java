...
//		javaFunctions(sampleStockRDD).writerBuilder("finance", "stocks", mapToRow(StockSample.class)).saveToCassandra();
		
		
//		CassandraJavaUtil.mapToRow(StockSample.class)
//		RowWriterFactory<StockSample> rowWriterFactory
//		javaFunctions(sampleStocks).writerBuilder("finance", "stocks", rowWriterFactory).saveToCassandra();
		
        
//		javaFunctions(sampleStockRDD, StockSample.class);
		
//		JavaRDD<String> cassandraRowsRDD = javaFunctions(jssc).cassandraTable("finance", "stocks").map(new Function<CassandraRow, String>() {
//			@Override
...
//		System.out.println("Data as CassandraRows: \n" + cassandraRowsRDD.count());
        ...
        JavaPairDStream<String,Double> avg = stocks
...
//        max.print();
        ...
        JavaPairDStream<String,Stock> newestStock = symbolStock.reduceByKeyAndWindow((x,y) -> (x.getQuote().getLastTradeTime().compareTo(y.getQuote().getLastTradeTime()) > 0 ? x : y)
        		,WINDOW_DURATION, SLIDE_DURATION);
        
        JavaPairDStream<String,Stock> oldestStock = symbolStock.reduceByKeyAndWindow((x,y) -> (x.getQuote().getLastTradeTime().compareTo(y.getQuote().getLastTradeTime()) < 0 ? x : y)
        		,WINDOW_DURATION, SLIDE_DURATION);
        
        JavaPairDStream<String, Tuple2<Stock, Stock>> join = newestStock.join(oldestStock);
        
        JavaPairDStream<String,Double> percentageVariation = join.mapToPair(line -> new Tuple2<>(line._1(),
        	line._2()._1().getQuote().getPrice().doubleValue() / line._2()._2().getQuote().getPrice().doubleValue()));
