...
import it.himyd.stock.finance.yahoo.histquotes.HistoricalQuote;
import it.himyd.stock.finance.yahoo.histquotes.Interval;
import it.himyd.stock.finance.yahoo.quotes.stock.*;
...
public class Stock implements Serializable {
	...
	public static Stock fromLineString(String line) {
		Stock stock = new Stock();
		String[] details = line.split("\\|");
		String symbol = details[0];
		String price = details[1];
		String volume = details[2];
		String lastTradeMillis = details[3];

		stock.setSymbol(symbol);
		stock.getQuote().setPrice(new BigDecimal(price));
		stock.getQuote().setVolume(Long.parseLong(volume));

		Calendar gc = GregorianCalendar.getInstance();
		if (!lastTradeMillis.equals("null")) {
			gc.setTimeInMillis(Long.parseLong(lastTradeMillis));
			stock.getQuote().setLastTradeTime(gc);
		} else {
			stock.getQuote().setLastTradeTime(gc);
		}
	...
	public String toString() {
		...
		sb.append(this.name);
		sb.append(" | ");
		sb.append(this.quote.getPrice().toString());
		return sb.toString();
	}
